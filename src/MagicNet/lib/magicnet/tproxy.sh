...
magicnet_tproxy_cleanup_family() {
    ...
    if magicnet_cmd_exists "$_cmd"; then
        ...
        "$_cmd" -t mangle -D OUTPUT -j MAGICNET_TPROXY_OUTPUT 2>/dev/null || true
        ...
        "$_cmd" -t mangle -X MAGICNET_TPROXY_OUTPUT 2>/dev/null || true
    fi
...
magicnet_tproxy_apply_family() {
    ...
    if [ "$_cmd" = "ip6tables" ]; then
        "$_cmd" -t mangle -A MAGICNET_TPROXY_OUTPUT -d ::1/128 -j RETURN 2>/dev/null || true
    else
    ...
    if [ "$_cmd" = "ip6tables" ]; then
        ...
        for _cidr in fc00::/7 fe80::/10 ff00::/8; do
            "$_cmd" -t mangle -A MAGICNET_TPROXY -d "$_cidr" -j RETURN 2>/dev/null || true
            "$_cmd" -t mangle -A MAGICNET_TPROXY_OUTPUT -d "$_cidr" -j RETURN 2>/dev/null || true
        done
    else
        for _cidr in 0.0.0.0/8 10.0.0.0/8 100.64.0.0/10 \
    ...
    "$_cmd" -t mangle -A MAGICNET_TPROXY_OUTPUT -p tcp -j MARK --set-mark "$_mark" 2>/dev/null || return 1
    "$_cmd" -t mangle -A MAGICNET_TPROXY_OUTPUT -p udp -j MARK --set-mark "$_mark" 2>/dev/null || return 1
    ...
    "$_cmd" -t mangle -C OUTPUT -j MAGICNET_TPROXY_OUTPUT >/dev/null 2>&1 ||
        "$_cmd" -t mangle -A OUTPUT -j MAGICNET_TPROXY_OUTPUT >/dev/null 2>&1 || return 1
