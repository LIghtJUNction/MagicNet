...
magicnet_warp_apply_singbox() {
    ...
    _jq="$(magicnet_require_jq "packaged jq is unavailable; WARP apply rejected")" || {
        unset _config
        return 1
    }
    ...
    if magicnet_warp_enabled && magicnet_warp_configured; then
        magicnet_jq_install_config "$_config" "$_tmp" "$_jq" --slurpfile warp "$_endpoint" -e '
          def with_warp($items):
        ...
        magicnet_jq_install_config "$_config" "$_tmp" "$_jq" -e '
