...
MagicNet 是一个 Android root 透明代理模块，用 `sing-box`、MagicNet eBPF 和 TUN 兜底路径接管设备侧应用流量。它面向网络连通性验证、DNS 泄露检查、透明流量治理、分应用策略和自动化诊断。
...
- 单核心：只启动和维护 `sing-box`。
- 默认 `auto`：eBPF 数据面完整可用才启用，否则回退 TUN。
- eBPF：覆盖本机 TCP、DNS TCP 53、DNS UDP 53。
- TUN：覆盖完整透明代理兜底路径。
- 诊断：CLI / WebUI / MCP / support bundle 汇总运行状态、路由、DNS、eBPF 与 netd 信息。

## 功能

- Android root 透明流量接管。
- `auto` / TUN / eBPF 三种透明流量策略。
...
eBPF 是 MagicNet 的目标数据面，不是 sing-box 原生开关。它依赖 Android 内核 BPF / cgroup BPF 能力，并通过 MagicNet loader 完成 TCP bridge、DNS 53 重定向和 netd BPF 状态检查。DNS 53 会进入 sing-box 本地 `direct` DNS 入站，再由 `hijack-dns` 交给 sing-box DNS 模块。

- 可用性以 `cli health` 和 `cli ebpf status` 为准。
- 全量 UDP 不伪装成已完成能力；QUIC、游戏、WebRTC、VoIP 仍由 TUN 兜底。
- 全量 UDP/QUIC 需要单独的 UDP bridge、原始目的地映射和 SOCKS5 UDP ASSOCIATE 回包路径；不能把 DNS `sendmsg` 改写直接扩展到所有 UDP。
- `dns_udp4=attached` / `dns_udp6=attached` 需要用物理出口抓包验证没有 `port 53 or port 853` 泄露。
...
MagicNet 借鉴成熟 Android root 网络模块的“核心启动器、透明规则分层、配置合法性检查、手动控制、日志可追踪”思路，但把对外定位收敛为网络安全分析与设备侧流量审计。eBPF、IPSET_LKM 和 MCP 都是显式启用或可选增强，不会在默认路径里隐式接管用户网络。
