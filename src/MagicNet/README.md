...
- `Bypass TUN` 会按每个 Android 用户动态解析应用 UID，并让该 UID 同时绕过 MagicNet 的 TUN 与 DNS 捕获。应用离开 MagicNet 后会使用系统上游网络；如果上游网络或另一个 VPN 能访问 Google，加入 Bypass 后的 Chrome 仍然可以访问。
