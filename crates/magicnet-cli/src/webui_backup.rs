...
mod tests {
    ...
    fn sourced_conf_schema_accepts_known_values_and_rejects_unknown_or_unsafe_ones() {
        ...
        for invalid in [
            "MAGICNET_TRANSPARENT_MODE=proxy\n",
            "MAGICNET_TRANSPARENT_MODE=external\n",
            "MAGICNET_TRANSPARENT_MODE=hybrid\n",
        ] {
            assert!(!sourced_conf_content_matches_schema(transparent, invalid));
        }
    }
    ...
    #[test]
    fn restore_rejects_legacy_transparent_modes_without_writing() {
        let app = temp_app();
        let mode = app.moddir.join(".config/magicnet/transparent-mode.conf");
        fs::create_dir_all(mode.parent().expect("mode parent")).unwrap();
        fs::write(&mode, "MAGICNET_TRANSPARENT_MODE=tun\n").unwrap();

        for payload in [
            "MAGICNET_TRANSPARENT_MODE=proxy\n",
            "MAGICNET_TRANSPARENT_MODE=external\n",
            "MAGICNET_TRANSPARENT_MODE=external-tun\n",
            "MAGICNET_TRANSPARENT_MODE=hybrid\n",
        ] {
            let text = format!(
                "MagicNet backup v2\n--- .config/magicnet/transparent-mode.conf\n{payload}"
            );
            let err = restore_backup(&app, &text).expect_err("legacy transparent mode");
            assert!(
                err.contains("transparent-mode.conf"),
                "unexpected error for {payload:?}: {err}"
            );
            assert_eq!(
                fs::read_to_string(&mode).unwrap(),
                "MAGICNET_TRANSPARENT_MODE=tun\n"
            );
        }
    }
