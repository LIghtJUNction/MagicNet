...
fn block_list(app: &App) {
    ...
    println!("enabled={}", conf.get("MAGICNET_BLOCK_ENABLED").map(String::as_str).unwrap_or("1"));
    println!("community={}", conf.get("MAGICNET_BLOCK_COMMUNITY_ENABLED").map(String::as_str).unwrap_or("1"));
    println!("url={}", conf.get("MAGICNET_BLOCK_URL").map(String::as_str).unwrap_or(default_block_url()));
    ...
    for line in clean_lines(dir.join("community-ban-rules.list")).into_iter().filter(|line| !allow.contains(line)) {
        println!("  {line}");
    }
...
fn block_toggle(app: &App, action: &str) -> Result<(), String> {
    ...
    conf.insert("MAGICNET_BLOCK_ENABLED".to_string(), if action == "enable" { "1" } else { "0" }.to_string());
    write_block_conf(app, &conf)?;
    run_magicnet_function(app, "magicnet_block_apply")?;
    println!("[info] Blocklist {}", if action == "enable" { "enabled" } else { "disabled" });
    Ok(())
}
...
fn block_community(app: &App, args: &[String]) -> Result<(), String> {
    ...
    let mut conf = block_conf_values(app);
    conf.insert("MAGICNET_BLOCK_COMMUNITY_ENABLED".to_string(), value.to_string());
    write_block_conf(app, &conf)?;
    run_magicnet_function(app, "magicnet_block_apply")?;
    println!("[info] Community blocklist {}", if value == "1" { "enabled" } else { "disabled" });
    Ok(())
}
...
fn block_domain(app: &App, args: &[String]) -> Result<(), String> {
    ...
    update_line(conf_dir(app).join("block-domain-suffix.list"), domain, args[0] == "add-domain")?;
    run_magicnet_function(app, "magicnet_block_apply")?;
...
fn block_allow(app: &App, args: &[String]) -> Result<(), String> {
    ...
    update_line(conf_dir(app).join("block-allow-rules.list"), &normalize_block_rule(rule), args[0] == "allow-rule")?;
    run_magicnet_function(app, "magicnet_block_apply")?;
...
fn block_conf_values(app: &App) -> HashMap<String, String> {
    ...
    conf.entry("MAGICNET_BLOCK_ENABLED".to_string()).or_insert_with(|| "1".to_string());
    conf.entry("MAGICNET_BLOCK_COMMUNITY_ENABLED".to_string()).or_insert_with(|| "1".to_string());
    conf.entry("MAGICNET_BLOCK_URL".to_string()).or_insert_with(|| default_block_url().to_string());
    conf
}
...
fn write_block_conf(app: &App, conf: &HashMap<String, String>) -> Result<(), String> {
    write_kv(
        ...
        &[
            ("MAGICNET_BLOCK_ENABLED", conf.get("MAGICNET_BLOCK_ENABLED").cloned().unwrap_or_else(|| "1".to_string())),
            ("MAGICNET_BLOCK_COMMUNITY_ENABLED", conf.get("MAGICNET_BLOCK_COMMUNITY_ENABLED").cloned().unwrap_or_else(|| "1".to_string())),
            ("MAGICNET_BLOCK_URL", conf.get("MAGICNET_BLOCK_URL").cloned().unwrap_or_else(|| default_block_url().to_string())),
        ],
    )
}
