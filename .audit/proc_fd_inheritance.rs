...
fn blocked_proc_worker_releases_unrelated_descriptors() {
    ...
    let root = std::env::temp_dir().join(format!("magicnet-proc-fds-{}-{nonce}", std::process::id()));
    ...
    let mut writer = fs::OpenOptions::new().write(true).create_new(true).open(&binary).unwrap();
    writer.write_all(b"#!/bin/sh\nexit 0\n").unwrap();
    fs::set_permissions(&binary, fs::Permissions::from_mode(0o700)).unwrap();
    let lock_path = root.join("lock");
    let lock = fs::OpenOptions::new().read(true).write(true).create_new(true).open(&lock_path).unwrap();
    ...
    let reader = std::thread::spawn(move || {
        sender.send(unsafe { libc::syscall(libc::SYS_gettid) }).unwrap();
    ...
    let guard = loop {
        match fs::OpenOptions::new().write(true).custom_flags(libc::O_NONBLOCK).open(root.join("blocked-proc")) {
            Ok(file) => break file,
    ...
    let inherited: Vec<_> = fs::read_dir(format!("/proc/{}/fd", pids[0])).unwrap()
        .map(|entry| fs::read_link(entry.unwrap().path()).unwrap()).collect();
    drop(writer);
    drop(lock);
    ...
    assert!(!inherited.contains(&binary), "proc worker retained an executable writer");
    assert!(!inherited.contains(&lock_path), "proc worker retained an unrelated lock");
    assert!(executable_result.unwrap().success(), "closed executable remained busy");
    assert_eq!(lock_result, 0, "closed parent lock remained owned: {lock_error}");
}
