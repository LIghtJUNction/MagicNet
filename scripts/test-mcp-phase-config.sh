...
assert_started_once() {
    grep -qx 'mcp start' "$call_log" || fail "expected one MCP CLI start"
    [ "$(wc -l < "$call_log")" -eq 1 ] || fail "expected exactly one MCP CLI start"
}

assert_not_started() {
    [ ! -s "$call_log" ] || fail "MCP CLI must not start"
}
