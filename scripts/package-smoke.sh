...
if route_rules[25] != {
    ...
    raise SystemExit("route rule 25 Microsoft network-test suffixes changed")
if route_rules[27] != {
    "domain_suffix": foreign_network_test_suffixes,
    "outbound": "network-test",
}:
    raise SystemExit("route rule 27 foreign network-test suffixes changed")
if foreign_connectivity_rule["domain_suffix"] != (
    route_rules[25]["domain_suffix"] + route_rules[27]["domain_suffix"]
):
    raise SystemExit("foreign network-test DNS suffixes must equal route rules 25 + 27")
apple_icloud_rule = {
    "domain_suffix": [
        "apple.com",
        "icloud.com",
...
if dns_rules[19] != foreign_priority_dns_rule:
    raise SystemExit("packaged exact 56-domain foreign-priority DNS rule must remain at index 19")
...
if (
    ...
    raise SystemExit(
        ...
        f"domestic={domestic_connectivity_rules} foreign={foreign_connectivity_rules} "
...
if route_rules[47] != foreign_priority_route_rule:
    raise SystemExit("packaged exact 56-domain foreign-priority route must remain at index 47")
if route_rules[47]["domain_suffix"] != dns_rules[19]["domain_suffix"]:
...
if (
    route_rules[29].get("outbound") != "cn-direct"
    or not explicit_domain_matches(route_rules[29], "mmstat.com")
    or any(explicit_domain_matches(rule, "mmstat.com") for rule in route_rules[:29])
    or recursively_effective_outbound(route_rules[29]["outbound"]) != "direct"
):
    raise SystemExit("packaged mmstat.com route must first-match index 29 cn-direct/direct")
