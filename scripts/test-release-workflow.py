...
class WorkflowStructureTest(unittest.TestCase):
    def test_build_and_release_share_one_job_and_exact_checkout(self):
        ...
        installer = next(step for step in steps
                         if step["name"] == "Package core, components and offline full module")
        self.assertNotIn("kam installer", text)
        self.assertIn("scripts/package-components.py", installer["run"])
        self.assertIn("scripts/test-downloader-installer.py dist/magicnet_installer.zip", installer["run"])
        self.assertLess(installer["run"].index("scripts/package-components.py"),
                        installer["run"].index("scripts/test-downloader-installer.py"))
        ...
        self.assertLess(steps.index(signing), steps.index(release))
