export default {
  "状态待确认": ["Status unconfirmed", "Состояние не подтверждено"],
  "节点准备": ["Node preparation", "Подготовка узлов"],
  "代理链配置": ["Proxy chain configuration", "Настройка цепочки прокси"],
  "透明代理配置": ["Transparent proxy configuration", "Настройка прозрачного прокси"],
  "热点策略": ["Hotspot policy", "Политика точки доступа"],
  "Tailscale 配置": ["Tailscale configuration", "Настройка Tailscale"],
  "应用分流": ["App routing", "Маршрутизация приложений"],
  "WARP 配置": ["WARP configuration", "Настройка WARP"],
  "Tailscale 授权": ["Tailscale authorization", "Авторизация Tailscale"],
  "配置校验": ["Configuration validation", "Проверка конфигурации"],
  "路由状态读取": ["Route state inspection", "Чтение состояния маршрутов"],
  "内核启动": ["Core launch", "Запуск ядра"],
  "路由归属验证": ["Route ownership verification", "Проверка принадлежности маршрутов"],
  "网络初始化": ["Network initialization", "Инициализация сети"],
  "启动停在：{stage}": ["Startup stopped at: {stage}", "Запуск остановлен: {stage}"],
  "此步骤未完成，后续启动已中止。查看输出中的前置错误，再处理对应设置。": ["This step did not complete, so startup stopped. Check the preceding error in Output before changing the corresponding settings.", "Этот шаг не завершён, запуск остановлен. Проверьте предшествующую ошибку в выводе перед изменением настроек."],
  "配置正在被其他任务占用": ["Configuration is in use", "Конфигурация занята"],
  "已达到配置锁等待上限。先查看后台任务，不要删除锁或反复重启。": ["The configuration lock wait limit was reached. Check the background task; do not delete the lock or repeatedly restart.", "Истекло время ожидания блокировки конфигурации. Проверьте фоновую задачу; не удаляйте блокировку и не перезапускайте многократно."],
  "启动失败，原因尚未捕获": ["Startup failed; cause not captured", "Запуск не удался; причина не получена"],
  "当前输出只有启动失败的汇总，不能据此判定为锁超时。请查看内核日志和带上下文的日志摘要。": ["The output only contains a startup failure summary; it does not prove a lock timeout. Check core logs and the log excerpt with context.", "В выводе есть только сводка сбоя запуска; это не доказывает тайм-аут блокировки. Проверьте журнал ядра и фрагмент журнала с контекстом."],
  "无法确认核心状态": ["Core status is unconfirmed", "Состояние ядра не подтверждено"],
  "未能读取进程状态，不会将它当作已停止。先刷新状态或查看诊断。": ["Process state could not be read; it is not treated as stopped. Refresh status or inspect diagnostics first.", "Не удалось прочитать состояние процесса; он не считается остановленным. Обновите состояние или откройте диагностику."],
  "核心进程存在，但服务就绪检查未通过。不要仅凭进程存在判断代理可用。": ["The core process exists, but service readiness checks did not pass. A running process alone does not prove proxy availability.", "Процесс ядра существует, но проверка готовности сервиса не пройдена. Наличие процесса не означает доступность прокси."],
  "核心进程存在，但尚未确认服务是否就绪。刷新状态不会重启服务。": ["The core process exists, but readiness is unconfirmed. Refreshing status will not restart the service.", "Процесс ядра существует, но готовность не подтверждена. Обновление состояния не перезапускает сервис."],
  "进程存在": ["Process present", "Процесс существует"],
  "已就绪": ["Ready", "Готов"],
  "未就绪": ["Not ready", "Не готов"],
  "在模块管理器中打开，连接设备后管理代理服务。": ["Open in your module manager to manage the proxy service on your device.", "Откройте в менеджере модулей для управления прокси на устройстве."],
  "服务已就绪。实际联网情况可在诊断中确认。": ["The service is ready. Verify actual connectivity in Diagnostics.", "Сервис готов. Проверьте доступ к сети в диагностике."],
  "启动前确认订阅已配置；现有配置和节点会保留。": ["Check your subscription before starting. Existing settings and nodes are kept.", "Проверьте подписку перед запуском. Текущие настройки и узлы сохранятся."],
  "先确认服务状态，再继续操作。刷新不会重启服务。": ["Check service status before proceeding. Refreshing does not restart the service.", "Проверьте состояние сервиса перед продолжением. Обновление не перезапускает сервис."],
  "代理服务": ["Proxy service", "Прокси-сервис"],
  "运行证据": ["Runtime evidence", "Данные о работе"],
  "服务检查": ["Readiness", "Готовность"],
  "实际模式": ["Effective mode", "Действующий режим"],
  "打开诊断": ["Open diagnostics", "Открыть диагностику"],
  "常用入口": ["Quick access", "Быстрый доступ"],
  "订阅与节点": ["Subscriptions & nodes", "Подписки и узлы"],
  "管理来源与更新": ["Manage sources and updates", "Источники и обновления"],
  "服务就绪后可打开": ["Available when service is ready", "Доступно, когда сервис готов"],
  "网络设置": ["Network settings", "Настройки сети"],
  "按需调整，无需反复重启": ["Adjust as needed, without repeated restarts", "Настройка без повторных перезапусков"],
