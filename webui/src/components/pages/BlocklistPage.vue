...
const { state, runCli, refreshBlock, openExternal, shellQuote, REPO } = useMagicNet();
  ...
  <div class="grid gap-4">
    <PageHeader overline="Community Banlist" title="联 ban 黑名单" description="本地规则和社区库排除都在这里。X 是排除社区规则，+ 是恢复阻断。">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <Button :loading="isRunning('update-block')" @click="withAction('update-block', async () => { await runCli('block update', '更新社区库'); await refreshBlock(true); })"><DownloadCloud :size="17" />更新社区库</Button>
