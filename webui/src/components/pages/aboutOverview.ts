...
export function pathFlowNodes(): AboutPathNode[] {
  return [
    ...
    { index: "04", label: t("策略 / 出口"), code: "OUT" },
  ];
}
...
export function dataPlaneFacts(): AboutFact[] {
  return [
    {
      code: "ROOT",
      title: t("Android root 工作台"),
      detail:
        t("MagicNet 通过模块 CLI 管理 sing-box，不调用应用侧 VpnService.establish()，也不会占用系统 VPN slot。"),
    },
    {
      code: "DATA",
      title: t("显式 tun | ebpf 数据面"),
      detail:
        t("默认 TUN 使用 magicnet0；eBPF 使用 local cgroup，并在确认真实下游接口后启用 shared TC。两种模式都进入同一 sing-box 策略与出口。"),
    },
    {
      code: "PROOF",
      title: t("以真实运行状态为准"),
      detail:
        t("真机是否成功，看 cli health 与 cli transparent status；只有 TUN 要求 magicnet0，eBPF 以 cgroup/TC attachment 为准。"),
    },
  ];
}
  ...
  return [
    {
      id: "dns",
      title: t("关闭私人 DNS"),
      detail: t("在系统设置中关闭私人 DNS / Private DNS，不要保留为自动。"),
    },
    {
      id: "sub",
      title: t("保存订阅或导入本地文件"),
      detail:
        t("在订阅页保存合法 URL，或导入 Clash YAML、分享链接、JSON 或文本订阅。"),
    },
    {
      id: "health",
      title: t("确认健康与透明数据面"),
      detail:
        t("健康检查没有核心/数据面阻塞项，transparent status 的 configured 与 effective 状态一致或明确标注 pending。"),
    },
  ];
}
...
export function successChecks(): AboutCheck[] {
  return [
    {
      command: "cli health",
      expect: t("没有核心或 Dataplane 阻塞项"),
    },
    {
      command: "cli transparent status",
      expect: t("configured/effective 模式与 attachment 状态明确"),
    },
    {
      command: "tun | ebpf",
      expect: t("TUN 存在 magicnet0；eBPF 报告 cgroup/TC 状态且不要求 magicnet0"),
    },
  ];
}
