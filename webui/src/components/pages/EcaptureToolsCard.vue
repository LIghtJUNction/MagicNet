...
async function runTcpdumpProbe(): Promise<void> {
  await runShell("timeout 10 tcpdump -i any -nn -c 30 'tcp port 443 or udp port 53 or tcp port 53 or tcp port 853 or udp port 853'", t("tcpdump 快速抓包"));
}

async function runEcaptureTlsCapture(): Promise<void> {
  await withAction("ecapture-tls-quick", async () => {
    state.output = await runCli("ecapture tls 8 all all", t("eCapture TLS 短抓"));
  });
}

async function runEcapturePcapCapture(command: string): Promise<void> {
  await withAction("ecapture-pcap-quick", async () => {
    state.output = await runCli(command, t("eCapture PCAP 短抓"));
  });
}
...
function tcpdumpProbe(): void {
  requestAction({
    key: "tcpdump-probe",
    get title() { return t("执行 tcpdump 探测"); },
    get detail() { return t("会短时间读取设备网络流量元数据，可能影响性能并暴露连接信息。"); },
...
function ecaptureTlsCapture(): void {
  requestAction({
    ...
    get title() { return t("执行 eCapture TLS 短抓"); },
    get detail() { return t("会运行 8 秒无证书 TLS 文本抓取，可能输出连接域名、进程或明文片段，仅在排障时使用。"); },
...
function ecapturePcapCapture(): void {
  ...
  requestAction({
    ...
    get title() { return t("执行 eCapture PCAP 短抓"); },
    get detail() { return t("会运行 8 秒 PCAP 抓取并写入模块日志目录，适合复查握手与 DNS 流量。"); },
    command,
...
function buildPcapCommand(reportError = true): string {
  ...
  if (!/^[A-Za-z0-9_.:-]{1,32}$/.test(ifname)) {
    if (reportError) state.output = t("PCAP 网卡名只能包含字母、数字、点、下划线、冒号和短横线。");
    return "";
  }
  const tokens = pcapFilter.value.trim().split(/\s+/).filter(Boolean);
  if (tokens.length > 12 || tokens.some((token) => !/^[A-Za-z0-9_.:-]+$/.test(token))) {
    if (reportError) state.output = t("PCAP 过滤器只能使用简单 tcp/udp/port/host 这类词元，最多 12 个。");
    return "";
  }
...
async function copyPcapCommand(): Promise<void> {
  ...
  state.output = copied ? t("PCAP 抓包命令已复制。") : t("剪贴板不可用，PCAP 抓包命令未复制。");
}
...
function buildPcapPlan(): { status: "ok" | "error"; summary: string; detail: string } {
  ...
  return {
    status: "ok",
    summary: t("将抓取 {ifname} 上的 {filter}，持续 8 秒。", { ifname, filter: tokens.join(" ") || t("全部") }),
    get detail() { return [
...
<template>
  <Card class="grid gap-3">
    <h3 class="inline-flex items-center gap-2 text-base font-semibold"><RadioTower :size="17" /> {{ t("无证书抓包") }}</h3>
    ...
    <div class="grid gap-2 sm:grid-cols-2">
      <Button :loading="isRunning('ecapture-status')" @click="withAction('ecapture-status', () => runCli('ecapture status', t('检查 eCapture')))">
        <RadioTower :size="16" />{{ t("eCapture 状态") }}
      </Button>
      <Button variant="secondary" :loading="isRunning('ecapture-version')" @click="withAction('ecapture-version', () => runCli('ecapture version', t('查看 eCapture 版本')))">
        {{ t("版本") }}
      </Button>
      <Button variant="secondary" :loading="isRunning('tcpdump-probe')" @click="tcpdumpProbe">
        <Network :size="16" />{{ t("tcpdump 探测") }}
      </Button>
      <Button variant="outline" :loading="isRunning('ecapture-tls-quick')" @click="ecaptureTlsCapture">
        TLS 8s
      </Button>
      <Button variant="outline" :loading="isRunning('ecapture-help-tls')" @click="withAction('ecapture-help-tls', () => runCli('ecapture help tls', t('查看 TLS 抓包帮助')))">
        {{ t("TLS 帮助") }}
      </Button>
      <Button variant="outline" :loading="isRunning('ecapture-help-pcap')" @click="withAction('ecapture-help-pcap', () => runCli('ecapture help pcap', t('查看 PCAP 抓包帮助')))">
        {{ t("PCAP 帮助") }}
      </Button>
    </div>
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <Button size="sm" variant="outline" :disabled="!pcapCommandPreview" @click="copyPcapCommand">
        <Copy :size="15" />{{ commandCopied ? t("已复制命令") : t("复制命令") }}
      </Button>
    </div>
    <div class="rounded-md border p-3" :class="pcapPlan.status === 'ok' ? 'mn-tone-ok' : 'border-[color-mix(in_srgb,var(--mn-coral)_70%,transparent)] bg-[color-mix(in_srgb,var(--mn-coral)_55%,var(--mn-carrier))] text-[var(--mn-danger)]'">
      <p class="text-sm font-semibold">{{ t("PCAP 抓包计划") }}</p>
    ...
    <code class="break-all rounded-md bg-[var(--mn-carrier-deep)] px-3 py-2 text-xs text-[var(--mn-ink-muted)]">
      {{ pcapCommandPreview || t("PCAP 参数无效，修正后再执行。") }}
    </code>
  </Card>
</template>
