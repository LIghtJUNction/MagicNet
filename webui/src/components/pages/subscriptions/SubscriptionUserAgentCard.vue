...
const userAgentError = computed(() => {
  if (/[\u0000-\u001f\u007f]/.test(normalizedUserAgent.value)) return t("User-Agent 不能包含控制字符。");
  if (userAgentBytes.value > 256) return t("User-Agent 最多 256 字节。");
  return "";
});
...
async function persistUserAgent(): Promise<boolean> {
  ...
  const result = await runCli(command, value ? t("保存订阅 User-Agent") : t("清除订阅 User-Agent"));
  ...
  await withAction("save-user-agent", async () => {
    ...
    if (props.configured) {
      await startBackgroundCli(
        "sub update-all",
        value ? t("使用自定义 User-Agent 刷新订阅") : t("使用默认 User-Agent 刷新订阅"),
        "",
        "sub update-all",
      );
    } else {
      state.output = value
        ? t("自定义 User-Agent 已保存，将在首次拉取订阅时使用。")
        : t("自定义 User-Agent 已清除，后续拉取将使用下载器默认值。");
    }
  });
}
...
<template>
  <Card>
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0">
        <h3 class="mt-1 text-base font-semibold text-[var(--mn-ink)]">{{ t("订阅 User-Agent") }}</h3>
      </div>
    </div>

    <div class="mt-4 flex flex-wrap gap-2" :aria-label="t('User-Agent 预设')">
      <button
    ...
    <label class="mt-4 block text-xs font-medium text-[var(--mn-ink-muted)]" for="subscription-user-agent">{{ t("自定义值") }}</label>
    <Input
    ...
    <p id="subscription-user-agent-help" class="mt-2 text-xs leading-5" :class="userAgentError ? 'text-[var(--mn-danger)]' : 'text-[var(--mn-ink-muted)]'">
      {{ userAgentError || t("可填写 sing-box、mihomo 或服务商要求的完整值；{value}/256 字节。", { value: userAgentBytes }) }}
    </p>
    ...
    <Button
      ...
      @click="saveUserAgent"
    >
      <Save :size="16" />{{ configured ? t("保存并刷新订阅") : t("保存 User-Agent") }}
    </Button>
  </Card>
</template>
