...
export function buildTrafficBudgetPlan(
  ...
  if (budgetGiB === null) {
    return plan("idle", "未设置预算", "输入剩余流量 GiB 后，可按真实采样速率估算消耗时间。", projectedBytes, averageTotal, 0, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
  }
  if (!summary.latest) {
    return plan("idle", "等待流量样本", "先采样一次或开启自动采样，预算预测才有数据基础。", projectedBytes, averageTotal, remainingBytes, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
  }
  if (confidence !== "normal") {
    return plan("idle", "样本不足，仅粗略估算", `已有样本窗口不足 30 秒；${horizonMinutes} 分钟消耗只作为瞬时速率外推，不做风险判定。`, projectedBytes, averageTotal, remainingBytes, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
  }
  if (projectedBytes >= budgetBytes) {
    return plan("danger", "预测会超出预算", `按当前平均速率，${horizonMinutes} 分钟内会用完输入预算。`, projectedBytes, averageTotal, remainingBytes, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
  }
  if (projectedBytes >= budgetBytes * 0.7) {
    return plan("warning", "接近预算上限", `按当前平均速率，${horizonMinutes} 分钟预计使用超过预算的 70%。`, projectedBytes, averageTotal, remainingBytes, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
  }
  return plan("ok", "预算压力较低", `按当前平均速率，${horizonMinutes} 分钟预计不会触及输入预算。`, projectedBytes, averageTotal, remainingBytes, timeToBudgetSeconds, confidence, summary.sampleCount, horizonMinutes);
}
