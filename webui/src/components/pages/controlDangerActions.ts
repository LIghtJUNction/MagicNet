...
export function singBoxToggleAction(running: boolean): ControlDangerAction {
  return {
    ...
    label: running ? t("停止 sing-box") : t("启动 sing-box"),
    message: running
      ? t("确认停止 sing-box？停止后流量可能无法继续通过 MagicNet。")
      : t("确认启动 sing-box？该操作不会主动停止一个已运行的内核。"),
    background: true,
  };
}
...
export function restartSingBoxAction(): ControlDangerAction {
  return {
    ...
    label: t("重启 sing-box"),
    message: t("确认重启 sing-box？当前连接可能会短暂中断。"),
    background: true,
  };
}
...
export function applyConfigAction(): ControlDangerAction {
  return {
    ...
    label: t("应用全部配置"),
    message:
      t("确认应用配置？运行中的 sing-box 会重启以读取最新配置，当前连接可能会短暂中断。"),
    background: false,
  };
}
...
export function repairAction(): ControlDangerAction {
  return {
    ...
    args: "repair",
    label: t("一键自修复"),
    message: t("确认执行一键自修复？它可能会改写配置并调整运行状态。"),
    background: false,
  };
}
...
export function stopAllServicesAction(): ControlDangerAction {
  return {
    ...
    args: "service stop",
    label: t("停止全部服务"),
    message: t("确认停止全部服务？流量可能无法继续通过 MagicNet。"),
    background: true,
  };
}
...
export function setTransparentModeAction(
  ...
  return {
    ...
    label: t("切换为 {targetLabel}", { targetLabel: targetLabel }),
    message:
      currentMode === "unknown"
        ? t("当前透明代理状态未知。确认切换为 {targetLabel}？MagicNet 会验证并启动目标模式；失败时将尝试恢复原配置。", { targetLabel: targetLabel })
        : t("确认从 {currentLabel} 切换为 {targetLabel}？MagicNet 会停止当前数据面，验证并启动目标模式；失败时将尝试恢复 {currentLabel}。", { currentLabel: currentLabel, targetLabel: targetLabel }),
    background: false,
  };
}
...
export function applyTransparentModeAction(): ControlDangerAction {
  return {
    ...
    label: t("应用编排模式"),
    message: t("确认重新应用编排模式并重启 sing-box？当前连接可能会短暂中断。"),
    background: false,
  };
}
