...
const modeHint = computed(() => draft.value.mode === "auto"
  ? t('自动模式使用 chain-auto URLTest 组选择落地出口；链路仍保持两跳，不会回落到直连。')
  : t('手动模式固定使用 chain-exit 当前选择的落地出口；链路失败不会静默回落到直连。')
);
...
async function loadData(silent = false): Promise<boolean> {
  const [statusText, nodesText] = await Promise.all([
    runCli("chain status", t('读取链式代理配置'), silent),
    runCli("node list", t('读取可用节点'), true),
  ]);
  const statusFailed = execFailed(statusText);
  const nodesFailed = execFailed(nodesText);
  if (statusFailed) {
    state.phase = "error";
    state.notice = t('读取链式代理配置失败');
  ...
  else if (!silent) state.output = t('链式代理状态已读取，但可用节点列表读取失败；请稍后刷新。');
  return true;
}
...
async function confirmApply(): Promise<void> {
  ...
  await withAction("proxy-chain-apply", async () => {
    for (const action of requested.actions) {
      const output = await runCli(actionCommand(action), t('链式代理：{value}', { value: action.label }));
      if (execFailed(output)) {
        state.output = t('链式代理配置未完成：{value}\n{output}', { value: actionDetail(action), output: output });
        await loadData(true);
        return;
      }
    }
    state.output = t('链式代理配置已保存并应用。');
    await loadData(true);
  });
}
...
<template>
  <div class="grid gap-4 md:gap-5">
    ...
    >
      <template #actions>
        ...
        <Badge :tone="statusTone">
          {{ status.enabled ? (runtimeActive ? t('运行中') : t('已启用')) : t('已停用') }}
        </Badge>
      </template>
    </PageHeader>
    ...
    <Card class="grid gap-4 !p-4 md:!p-6">
      <CardHeading
        :overline="t('当前配置')"
        :description="t('保存后重新应用配置；节点凭据仍由订阅管理。')"
      >
        <template #title>
          <span class="inline-flex items-center gap-2"><GitBranch :size="23" />{{ t('两跳链路') }}</span>
        </template>
        <Badge :tone="status.enabled ? 'success' : 'neutral'">{{ t('策略：') }}{{ status.enabled ? t('启用') : t('停用') }}</Badge>
        <Badge tone="neutral">{{ status.mode === "auto" ? t('自动出口') : t('手动出口') }}</Badge>
      </CardHeading>
      ...
      <label class="flex cursor-pointer items-start gap-3 rounded-[var(--mn-radius-md)] bg-[color-mix(in_srgb,var(--mn-ink)_5%,transparent)] p-4">
        ...
        />
        <span>
          <span class="block text-base font-semibold">{{ t('启用链式代理') }}</span>
          <span class="mt-1 block text-sm leading-6 text-[var(--mn-ink-muted)]">
            {{ t('启用后，默认 proxy 出口切换到 chain；停用后恢复普通代理选择器，不会删除节点配置。') }}
          </span>
        </span>
      </label>

      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile :label="t('策略状态')" :value="status.enabled ? t('已启用') : t('已停用')" />
        <StatTile :label="t('运行 API')" :value="status.runtime.available ? t('可用') : t('不可用')" />
        <StatTile :label="t('proxy 当前出口')" :value="status.runtime.proxy || '—'" mono />
        <StatTile :label="t('当前链路出口')" :value="status.runtime.exit || '—'" mono />
      </div>
    </Card>
    ...
    <Card class="grid gap-5 !p-4 md:!p-6">
      <CardHeading
        :overline="t('链路角色')"
        :title="t('选择两跳节点')"
        :description="t('节点来自当前 sing-box 订阅缓存；订阅更新后请点击刷新重新读取。')"
      >
        <Badge :tone="nodeLoadFailed ? 'warning' : 'neutral'">
          {{ nodeLoadFailed ? t('节点读取失败') : t('{count} 个可用节点', { count: nodes.length }) }}
        </Badge>
      </CardHeading>
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <label class="grid gap-2 text-sm font-medium">
          <span class="flex items-center justify-between gap-2">
            <span>{{ t('中转节点') }} <span class="font-normal text-[var(--mn-ink-muted)]">{{ t('· 第一跳') }}</span></span>
            <button
              v-if="draft.upstream"
              type="button"
          ...
          <select
            ...
            :disabled="!loaded || isRunning('proxy-chain-apply')"
          >
            <option value="">{{ t('未选择中转节点') }}</option>
            <option v-for="node in availableNodes" :key="`upstream-${node}`" :value="node">{{ sanitizeNodeText(node) }}</option>
          </select>
          <span class="text-xs leading-5 text-[var(--mn-ink-muted)]">{{ t('连接先到中转节点，再由它转发到落地节点。') }}</span>
        </label>

        <label class="grid gap-2 text-sm font-medium">
          <span class="flex items-center justify-between gap-2">
            <span>{{ t('落地节点') }} <span class="font-normal text-[var(--mn-ink-muted)]">{{ t('· 第二跳') }}</span></span>
            <button
              v-if="draft.exit"
              type="button"
              class="inline-flex min-h-8 items-center gap-1 rounded-full px-2 text-xs font-medium text-[var(--mn-ink-muted)] hover:bg-[color-mix(in_srgb,var(--mn-ink)_7%,transparent)] hover:text-[var(--mn-danger)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--mn-focus)]"
          ...
          <select
            ...
            :disabled="!loaded || isRunning('proxy-chain-apply')"
          >
            <option value="">{{ t('未选择落地节点') }}</option>
            <option v-for="node in availableNodes" :key="`exit-${node}`" :value="node">{{ sanitizeNodeText(node) }}</option>
          </select>
          <span class="text-xs leading-5 text-[var(--mn-ink-muted)]">{{ t('最终流量从落地节点访问目标站点。') }}</span>
        </label>
      </div>

      <div class="grid gap-3">
        <div class="text-sm font-medium">{{ t('落地选择模式') }}</div>
        <div class="grid gap-3 sm:grid-cols-2">
          <button
            ...
            @click="setMode(mode)"
          >
            <span class="block font-semibold">{{ mode === 'manual' ? t('手动模式') : t('自动模式') }}</span>
            <span class="mt-1 block text-xs leading-5">{{ mode === 'manual' ? t('固定使用 chain-exit 当前选择') : t('使用 chain-auto URLTest 选择') }}</span>
          </button>
        </div>
      ...
      <div v-else-if="dirty" class="flex items-center gap-2 rounded-[var(--mn-radius-md)] bg-[color-mix(in_srgb,var(--mn-cactus)_12%,transparent)] px-4 py-3 text-sm text-[var(--mn-success)]">
        <ShieldCheck :size="16" />{{ t('配置已修改，点击保存后才会写入设备。') }}
      </div>
      ...
      <Button
        ...
        @click="requestApply"
      ><Save :size="17" />{{ t('保存并应用链式代理') }}</Button>
    </Card>
    ...
    <Card class="grid gap-4 !p-4 md:!p-6">
      <CardHeading
        ...
        :overline="t('应用预览')"
        :title="t('确认配置变更')"
        :description="t('每次保存会按顺序执行 CLI 配置操作；如果中途失败，页面会重新读取已落盘状态。')"
      />
      <div v-if="pendingPlan" class="grid gap-3 rounded-[var(--mn-radius-md)] bg-[color-mix(in_srgb,var(--mn-oat)_45%,var(--mn-carrier))] p-4" role="alert">
        ...
        <div class="flex flex-wrap gap-2">
          <Button :loading="isRunning('proxy-chain-apply')" @click="confirmApply">{{ t('确认保存') }}</Button>
          <Button variant="outline" :disabled="isRunning('proxy-chain-apply')" @click="cancelApply">{{ t('取消') }}</Button>
        </div>
      </div>
      <p v-else class="rounded-[var(--mn-radius-md)] bg-[var(--mn-ivory)] px-4 py-3 text-sm text-[var(--mn-ink-muted)]">{{ dirty ? t('保存按钮会在这里生成操作预览。') : t('当前没有待应用的链式代理变更。') }}</p>
    </Card>

    <Card class="grid gap-3 !p-4 md:!p-6">
      <CardHeading size="md" :overline="t('运行观察')" :title="t('链路选择器状态')" />
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile
          v-for="item in [
      ...
      <p class="text-xs leading-5 text-[var(--mn-ink-muted)]">
        {{ t('这里显示的是设备当前 sing-box API 选择器状态；策略已启用但 API 不可用时，需要先检查 sing-box、cli transparent status 与当前数据面。') }}
      </p>
    </Card>
  </div>
</template>
