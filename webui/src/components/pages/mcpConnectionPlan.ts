import type { McpState } from "@/composables/parsers";
...
export function buildMcpConnectionPlan(input: McpConnectionPlanInput): McpConnectionPlan {
  ...
  if (!validPort(port)) checks.push("端口无效：MCP CLI 只接受 1-65535。");
  ...
  if (isWildcardBind(bind)) checks.push("监听所有地址：同一网络内设备侧端口可能被其他主机访问。");
  else if (isLocalBind(bind)) checks.push("仅本机监听：适合 adb forward 连接。");
  else checks.push(`自定义监听地址：确认设备侧 ${bind}:${port} 可绑定。`);

  const running = input.enabled && input.pid !== "stopped";
  const invalidPort = !validPort(port);
  const blocked = Boolean(activePortOwner && input.pid === "stopped");
  const title = invalidPort ? "MCP 端口格式错误" : blocked ? "MCP 端口需要处理" : running ? "MCP 鉴权连接计划" : "MCP 连接前需要启动";
  const summary = invalidPort
    ? "先修正端口后再保存或复制连接命令。"
    : blocked
  ...
  return {
    tone: invalidPort || blocked || !running || !input.secretSet ? "warning" : "success",
    title,
    summary,
    localUrl,
    adbForward,
