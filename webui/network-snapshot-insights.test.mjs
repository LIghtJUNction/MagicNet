...
try {
  const source = await readFile(
    new URL(
      "./src/components/pages/networkSnapshotInsights.ts",
      import.meta.url,
    ),
    "utf8",
  );
  const tone = await readFile(
    new URL("./src/lib/statusTone.ts", import.meta.url),
    "utf8",
  );
  ...
  const insights = await import(
    pathToFileURL(join(dir, "networkSnapshotInsights.mjs")).href
  );
  const tunInsight = (text) =>
    insights
      .buildNetworkSnapshotInsights(text)
      .find((item) => item.label === "TUN 接口");

  assert.equal(
    tunInsight("2: Meta: <POINTOPOINT,UP> state UNKNOWN")?.value,
    "not detected",
  );
  assert.equal(
    tunInsight("2: Meta: <POINTOPOINT,UP> state UNKNOWN")?.tone,
    "info",
  );
  assert.equal(
    tunInsight("8: magicnet0: <POINTOPOINT,UP> state UNKNOWN")?.value,
    "detected",
  );
  assert.equal(
