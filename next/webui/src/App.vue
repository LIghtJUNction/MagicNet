...
import { request, status as parseStatus, settings as parseSettings, capabilities as parseCapabilities, RpcError, type Settings, type Status, type Capabilities, object } from './protocol';
...
const messages: Record<string,string> = {
  ...
  invalid_share_link:'分享链接包含无效字段，原节点未被替换。', unsupported_plugin:'插件参数不受支持，原节点未被替换。', invalid_request:'请求格式无效。', request_conflict:'请求标识冲突，没有重复执行。', source_not_ready:'请先成功更新订阅，再应用配置。',
};
...
async function mutate(method: string, params: unknown = null) {
  ...
  try {
    const result = await send(request(method, params, saved.value.revision));
    const importing = method === 'sources.import' || method === 'sources.refresh';
    if (importing && (!object(result) || !Number.isSafeInteger(result.nodes) || !Number.isSafeInteger(result.rejected) || Number(result.nodes) < 0 || Number(result.rejected) < 0)) throw new RpcError('invalid_response', true);
    ...
    note.value = importing && object(result) ? `已保存 ${result.nodes} 个节点；${result.rejected} 个条目无法转换，未导入。尚未应用到运行配置。` : method.startsWith('sources.') || method === 'settings.replace' ? '已保存，尚未应用到运行配置。' : '操作已返回，请以当前状态为准。';
  } catch (e) { failure(e); }
  ...
  <div class="app-shell">
    ...
    <main id="main-content">
      ...
      <section v-if="page==='sources'" aria-label="订阅管理" class="stack">
        ...
        <article class="card"><h2>本地导入</h2><p>支持原生 / Clash JSON、YAML、分享链接列表和 Base64 订阅。仅导入可转换的节点，并报告拒绝数量；不会用订阅覆盖本机路由。</p><label for="local">订阅内容</label><textarea id="local" v-model="local" :disabled="!editable" rows="5" spellcheck="false" placeholder='{"outbounds": [...]}'></textarea><div class="actions"><button :disabled="!editable || !local.trim() || dirty" @click="mutate('sources.import',{body:local})">校验并保存本地节点</button></div></article>
      </section>
