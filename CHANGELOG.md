...
- Fix #121 by assigning ChatGPT WebView DNS answers to a dedicated FakeIP pool before CN GeoIP classification, and clear the zero-node bootstrap selector cache after the first successful subscription import so populated proxy groups cannot remain pinned to `block`.
