...
发布工作流先构建并发布 `downloader-template-v1.0.2`，再下载、导入模板，使用 Kam 初始化和构建安装器，验证后发布 `magicnet-installer-v1.0.2`。两个发布均不标记 Latest，确保 MagicNet 本体仍是下载器读取的最新版本。
...
kam init /tmp/magicnet_installer -t templates/downloader_template.tar.gz --id magicnet_installer --project-name 'MagicNet Installer' --author LIghtJUNction --version 1.0.2
