...
WEBUI_TARGET_DIR="$KAM_MODULE_ROOT/webroot"
...
if [ -f "$WEBUI_SRC_DIR/package.json" ]; then
    if command -v bun >/dev/null 2>&1; then
        (cd "$WEBUI_SRC_DIR" && bun install --frozen-lockfile && bun run build)
    else
        log_error "webui/package.json found but bun is not installed"
        exit 1
    fi
else
