...
CACHE_FILE="${STATE_DIR}/ecapture.archive"
...
if ! release_lock_lookup ecapture || ! release_lock_is_valid; then
    log_error "ecapture: invalid repository release lock"
    exit 1
fi
...
if hook_locked_cache_is_valid "$CACHE_FILE" "$VERSION_FILE" "$RELEASE_LOCK_TAG" "$RELEASE_LOCK_SHA256"; then
    log_info "ecapture: using verified locked cache ($RELEASE_LOCK_TAG)"
else
    ...
    if ! hook_download_locked_asset "$RELEASE_LOCK_REPO" "$RELEASE_LOCK_TAG" "$RELEASE_LOCK_ASSET" "$TMP_DIR" >/dev/null \
        ...
        log_error "ecapture: locked download or verification failed; existing target was not changed"
        exit 1
    fi
